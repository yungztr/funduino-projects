...
oled.println("OLED Screen Test");
    ...
    tone(piezo, 300);
    oled.println("Genta>>>Batu");
    oled.println("Furkan groesste Noob");
    oled.println("Silvan King in fortnite");
    oled.println("Samet and Silvan = <3");
    oled.println("Toni x Genta = LOVE<3");
    oled.println("Mili > Furkan in fn");
    delay(10000);
...
void testRGBStreifen() 
{
  ...
  delay(1500);
  noTone(piezo);
  for (int i = 1; i < 256; i = i + 48) {
      for (int j = 1; j < 256; j = j + 48) {
        for (int k = 1; k < 256; k = k + 48) {
          oled.println("Rot: ");
          oled.println(i);
          oled.println("Gruen: ");
          oled.println(j);
